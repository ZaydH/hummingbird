import logging
# abc = Abstract Base Class - Used by Python to indicate something is an abstract class
import abc
import sys
import socket
...
class AbstractWorker():
  ...
  def run(self):
    """
    Runs the worker where it accepts messages from the master and then executes them.  This method
    eventually terminates based on a message from the master.
    """
...
class AbstractTask():
  ...
  def __init__(self):
    """
    Only defines the task generator.  The user may add any additional features to this class
    that they need when generating or processing task results.
    """
    self.generator = self.__class__.task_generator()

  def get_next(self):
    """
    Extracts and returns the next task a worker will perform.  Note that the task must be
    PICKABLE.
