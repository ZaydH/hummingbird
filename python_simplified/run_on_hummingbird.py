...
class Task(AbstractTask):
  ...
  def task_generator():
    ...
    #
    BENCHMARK_FOLDER = "/hb/home/zhammoud/repos/sat_benchmarks/thurley/uf200/uf200-"
    for i in range(1, 5):
      yield BENCHMARK_FOLDER + ("%03d" % i) + ".cnf"
  ...
  def process_results(self, results_dict):
    ...
    with open("results.csv", "a") as fout:
      fout.write("%s,%d\n" % (results_dict["benchmark"], results_dict["count"]))
...
class Worker(AbstractWorker):
  ...
  def execute_task(self, task_msg):
    ...
    #
    sharpSAT = "/hb/home/zhammoud/repos/sharpSAT/build/Release/sharpSAT"
    bin_args = [sharpSAT]
    bin_args.append(task_msg)
    ...
    for i in range(0, len(output)):
      if output[i].startswith("# solutions"):
         results_dict = dict()
         results_dict["benchmark"] = task_msg
         results_dict["count"] = int(output[i+1])
         return results_dict
